...
public class CDNSpeedTester:IDisposable
{
    ...
    public async Task<CdnTestResult> TestAsync(CdnList config,string url, TimeSpan sampleDuration, long maxBytes = 2 * 1024 * 1024, CancellationToken cancellationToken = default)
    {
        if (config == null) throw new ArgumentNullException(nameof(config));
        if (sampleDuration <= TimeSpan.Zero) throw new ArgumentOutOfRangeException(nameof(sampleDuration));
        ...
        return new CdnTestResult(config.Url, error == null, totalBytes, elapsed, score, speed, error);
    }

    public async Task<IReadOnlyList<CdnTestResult>> TestAllAsync(IEnumerable<CdnList> configs,string url,IndexResource resource, TimeSpan sampleDuration, long maxBytes = 2 * 1024 * 1024, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var cfg in configs)
        {
            ...
            var result = await TestAsync(cfg,url2, sampleDuration, maxBytes, cancellationToken).ConfigureAwait(false);
            results.Add(result);
        }
        return results;
    }
...
public readonly record struct CdnTestResult(string url, bool Success, long DownloadBytes, long TimeMillis, double Score, double BytesPerSecond, Exception? Error)
{
